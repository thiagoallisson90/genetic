...
class Chromosome():  
  def init_chr(self, var_min, var_max, min_dist):
    ...
    self.rules01 = self.gen_rules01()
    self.rules02 = self.gen_rules02()
    self.rules = self.rules01 + self.rules02
    self.solution = {
      'v': self.vertexes,
      'r': self.rules,
    }
  ...
  def set_rules01(self, _r1, _r2):
    r1 = [i for i in _r1]
    r2 = [i for i in _r2]
    self.rules01 = r1 + r2

  def set_rules02(self, _r1, _r2):
    r1 = [i for i in _r1]
    r2 = [i for i in _r2]
    self.rules02 = r1 + r2
  ...
  def change_rules01(self, gene):
    sets = ['low', 'average', 'high']
    previous = self.rules01[gene]
    while previous == self.rules01[gene]:
      self.rules01[gene] = sets[np.random.randint(0, 3)]
  
  def change_rules02(self, gene):
    sets = ['low', 'average', 'high']
    previous = self.rules02[gene]
    while previous == self.rules02[gene]:
      self.rules02[gene] = sets[np.random.randint(0, 3)]
  ...
  def gen_rules01(self):
    sets = ['low', 'average', 'high']
    return [sets[np.random.randint(0, 3)] for _ in range(3)]

  def gen_rules02(self):
    sets = ['low', 'average', 'high']
    return [sets[np.random.randint(0, 3)] for _ in range(3)]
  ...
  def copy(self):
    ...
    chr.rules01 = [i for i in self.rules01]
    chr.rules02 = [i for i in self.rules02]
    chr.rules = np.concatenate((chr.rules01, chr.rules02))
    self.solution = {
      'v': chr.vertexes,
      'r': chr.rules,
    }
  ...
  def __str__(self):
    return f'vertexes={self.vertexes};rules={self.rules};fitness={self.fitness}.'